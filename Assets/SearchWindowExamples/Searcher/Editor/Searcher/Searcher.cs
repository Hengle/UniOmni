...
namespace UnityEditor.Searcher
{
    public class Searcher
    {
            ...
            ISearcherAdapter adapter)
        {
            ...
            int databaseId = 0;
            foreach (SearcherDatabaseBase database in databases)
            {
            ...
            this.adapter = adapter ?? new SearcherAdapter(title);
        }
        ...
        public IEnumerable<SearcherItem> Search(string query)
        {
            ...
            float maxScore = 0;
            foreach (SearcherDatabaseBase database in m_Databases)
            {
                float localMaxScore = 0;
                var localResults = database.Search(query, out localMaxScore);
